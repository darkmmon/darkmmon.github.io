...
function checkNextMove(BoardState: number[][], move: number) {
  return BoardState[move].reduce((acc, cur) => acc && cur !== 0, true)
    ? 0
    : move + 1;
}
  ...
  if (nextMove != 0) {
    for (let i = 0; i < 9; i++) {
      if (BoardState[moveidx][i] !== 0) {
        ...
        const newBoardState = updateStatesOnMove(
          [moveidx, i],
          player,
          BoardState,
        );
        ...
        if (
          player == 1 ? moveValue > bestMoveValue : moveValue < bestMoveValue
        ) {
